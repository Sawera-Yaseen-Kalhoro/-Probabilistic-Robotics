...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def fs(self, xsk_1, usk):  # input velocity motion model with velocity noise
        ...
        **To be completed by the student**.
        ...
        nu_d = np.array([[usk[0]],[0],[usk[1]]]) # desired velocity

        wsk = np.reshape(np.random.normal(loc=np.zeros_like(self.Qsk), scale=self.Qsk).diagonal(), (3,1)) # sample Gaussian for the acceleration perturbation
    ...
    def ReadEncoders(self):
        ...
        **To be completed by the student**.
        ...
        # compute Rsk = covariance matrix of pulse reading
        Rsk = np.reshape(np.random.normal(loc=np.zeros_like(self.Re), scale=self.Re).diagonal(), (2,1))
    ...
    def ReadCompass(self):
        ...
        return self.xsk[2] + R_yaw, self.v_yaw_std
