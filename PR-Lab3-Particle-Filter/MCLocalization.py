...
class MCLocalization(ParticleFilter):
    ...
    def Prediction(self, u, Q):
        ...
        # u = input vector = displacement in x and y directions
        # Q = covariance matrix

        particles_new = [] # initialize a list of new particles (after applying motion model)

        for particle in self.particles:
            noise = np.random.multivariate_normal([0,0],Q).reshape((2,1))
            particles_new.append(self.MotionModel(particle,u,noise))
        
        self.particles = np.array(particles_new)
